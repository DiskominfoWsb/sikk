...
class Operator_Controller extends MY_Controller
{
    public function __construct()
    {
        ...
        {
            /* Load */
            $this->load->config('operator/dp_config');
            $this->load->library('admin/page_title');
            $this->load->library('admin/breadcrumbs');
            $this->load->model('operator/core_model');
            $this->load->helper('menu');
            $this->lang->load(array('operator/main_header', 'operator/main_sidebar', 'operator/footer', 'operator/actions'));

            /* Load library function  */
            $this->breadcrumbs->unshift(0, $this->lang->line('menu_dashboard'), 'operator/dashboard');

            /* Data */
            $this->data['title']       = $this->config->item('title');
            $this->data['title_lg']    = $this->config->item('title_lg');
            $this->data['title_mini']  = $this->config->item('title_mini');
            $this->data['admin_prefs'] = $this->prefs_model->operator_prefs();
            $this->data['user_login']  = $this->prefs_model->user_info_login($this->ion_auth->user()->row()->id);
